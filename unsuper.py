...
# Visualize PCA using Plotly
df_pca = pd.DataFrame(X_pca, columns=['PC1', 'PC2'])
df_pca['label'] = news_data['preprocessed_news_text'].index

fig = px.scatter(df_pca, x='PC1', y='PC2', text='label', title='PCA of News Data')
...
def plot_interactive_cluster(labels, title):
    df_pca['cluster'] = labels
    fig = px.scatter(df_pca, x='PC1', y='PC2', color='cluster', text='label', title=title)
    fig.update_traces(textposition='top center', marker=dict(size=10))
    fig.show()

plot_interactive_cluster(kmeans_labels, 'K-Means Clustering')
plot_interactive_cluster(kmedoids_labels, 'K-Medoids Clustering')
plot_interactive_cluster(hierarchical_labels, 'Agglomerative Clustering')
plot_interactive_cluster(gmm_labels, 'GMM Clustering')
plot_interactive_cluster(dbscan_labels, 'DBSCAN Clustering')
