...
def extract_keywords(df):
    def text_rank_keywords(text):
        ...
        summary = summarizer(parser.document, 10)  # Adjust the number of keywords if needed
        keywords = [str(sentence) for sentence in summary]
        return ' '.join(keywords) if keywords else 'no_keywords'

    df['keywords'] = df['post_text'].apply(text_rank_keywords)
    return df
...
def apply_classifier(features, labels):
    clf = LogisticRegression(max_iter=300)  # Increase max_iter to 300
